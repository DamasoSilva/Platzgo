...
async function resolveEstablishmentIdBySlug(rawSlug: string): Promise<string | null> {
  ...
  const direct = await prisma.establishment.findFirst({ where: { slug: normalized }, select: { id: true } });
  if (direct) return direct.id;
  const candidates = await prisma.establishment.findMany({ select: { id: true, name: true, slug: true } });
  const matches = candidates.filter((c) => slugify(c.name) === normalized);
  if (matches.length !== 1) return null;
  const match = matches[0];
  if (!match.slug) {
    try { await prisma.establishment.update({ where: { id: match.id }, data: { slug: normalized } }); } catch { /* ignore */ }
  }
  return match.id;
}
...
export async function generateMetadata(props: { params: { slug: string } | Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const est = await prisma.establishment.findUnique({ where: { id: estId }, select: { name: true, description: true } });
  if (!est) return { title: "Estabelecimento" };
  return { title: `${est.name} • PlatzGo!`, description: est.description ?? `Agende horários em ${est.name}.` };
}
...
export default async function EstablishmentSlugPage(props: {
  ...
  const est = await prisma.establishment.findUnique({
    where: { id: estId },
    select: {
      id: true, name: true, description: true, photo_urls: true, address_text: true,
